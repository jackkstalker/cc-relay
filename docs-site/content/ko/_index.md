---
title: CC-Relay
layout: hextra-home
---

<div class="landing-page">

<div class="custom-hero">
  <h1 class="hero-title">CC-Relay</h1>
  <p class="hero-subtitle">
    Claude Code 멀티 모델 스토리를 재정의합니다
  </p>
  <div class="hero-buttons">
    <a href="docs/getting-started/" class="hero-button hero-button-primary">시작하기</a>
    <a href="https://github.com/omarluq/cc-relay" class="hero-button hero-button-secondary">
      <svg class="github-icon" viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>
      GitHub
    </a>
  </div>
</div>

<div class="mt-6 mb-6">
{{< hextra/feature-grid >}}
  {{< hextra/feature-card
    title="멀티 프로바이더 지원"
    subtitle="하나의 엔드포인트로 Anthropic과 Z.AI에 연결 (더 많은 프로바이더 지원 예정)"
  >}}
  {{< hextra/feature-card
    title="SSE 스트리밍"
    subtitle="Claude Code와 완벽하게 호환되는 Server-Sent Events 지원"
  >}}
  {{< hextra/feature-card
    title="다중 API 키"
    subtitle="프로바이더당 여러 API 키를 풀링하여 처리량 향상"
  >}}
  {{< hextra/feature-card
    title="유연한 인증"
    subtitle="Claude Code 구독 사용자를 위한 API 키 및 Bearer 토큰 지원"
  >}}
  {{< hextra/feature-card
    title="Claude Code 통합"
    subtitle="내장된 설정 관리로 한 번의 명령으로 설정 완료"
  >}}
  {{< hextra/feature-card
    title="Anthropic API 호환"
    subtitle="클라이언트 변경 없이 바로 사용 가능한 드롭인 대체"
  >}}
{{< /hextra/feature-grid >}}
</div>

<div class="info-box">
  <div class="info-box-title">
    <span class="info-icon">⚡</span>
    현재 기능
  </div>
  <div class="info-box-content">
    <div class="feature-item">
      <span class="feature-icon">🔑</span>
      <div>
        <strong>다중 API 키</strong>
        <p>프로바이더당 여러 키를 풀링하여 처리량 향상</p>
      </div>
    </div>
    <div class="feature-item">
      <span class="feature-icon">🔐</span>
      <div>
        <strong>다중 인증 지원</strong>
        <p>유연한 접근을 위한 API 키 및 Bearer 토큰 인증</p>
      </div>
    </div>
    <div class="feature-item">
      <span class="feature-icon">📡</span>
      <div>
        <strong>SSE 스트리밍</strong>
        <p>Claude Code의 실시간 스트리밍과 완벽한 호환성</p>
      </div>
    </div>
    <div class="feature-item">
      <span class="feature-icon">🎛️</span>
      <div>
        <strong>디버그 로깅</strong>
        <p>문제 해결을 위한 상세한 요청/응답 로깅</p>
      </div>
    </div>
    <div class="feature-item">
      <span class="feature-icon">⚙️</span>
      <div>
        <strong>환경 변수</strong>
        <p>YAML에서 ${VAR} 확장을 통한 안전한 설정</p>
      </div>
    </div>
    <div class="feature-item">
      <span class="feature-icon">🚀</span>
      <div>
        <strong>간편한 설정</strong>
        <p>cc-relay config cc init 한 번의 명령으로 Claude Code 통합</p>
      </div>
    </div>
  </div>
</div>

<div class="section-box">
  <h2 class="section-title">빠른 시작</h2>

<div class="terminal-container">
  <div class="terminal-header">
    <div class="terminal-buttons">
      <span class="terminal-button close"></span>
      <span class="terminal-button minimize"></span>
      <span class="terminal-button maximize"></span>
    </div>
    <div class="terminal-title">Terminal — bash</div>
  </div>
  <div class="terminal-body">
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-1"># 설치</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-2">go install github.com/omarluq/cc-relay@latest</span>
    </div>
    <div class="terminal-line terminal-output typing-3">
      <span class="terminal-success">✓ installed cc-relay@latest</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-4"># 설정 초기화</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-5">cc-relay config init</span>
    </div>
    <div class="terminal-line terminal-output typing-6">
      <span class="terminal-success">✓ Config created at ~/.config/cc-relay/config.yaml</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-7"># Claude Code 통합 설정</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-8">cc-relay config cc init</span>
    </div>
    <div class="terminal-line terminal-output typing-9">
      <span class="terminal-success">✓ Claude Code configured to use cc-relay</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-10"># 서버 실행</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-11">cc-relay serve</span>
    </div>
    <div class="terminal-line terminal-output typing-12">
      <span class="terminal-info">→ Server started on http://localhost:8787</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-13"># Claude Code 사용 시작</span>
    </div>
    <div class="terminal-line">
      <span class="terminal-prompt">$</span>
      <span class="terminal-command typing-14">claude</span>
    </div>
    <div class="terminal-line terminal-output typing-15">
      <span class="terminal-success">✓ Connected via cc-relay</span>
      <span class="terminal-cursor"></span>
    </div>
  </div>
</div>
</div>

<div class="section-box">
  <h2 class="section-title">아키텍처</h2>
  <p class="section-description">CC-Relay는 LLM 클라이언트와 백엔드 프로바이더 사이에서 Anthropic API와 완벽하게 호환되는 요청을 프록시합니다</p>

<div class="architecture-diagram">
  <div class="arch-layer">
    <div class="arch-layer-title">클라이언트 레이어</div>
    <div class="arch-node arch-node-client">
      <div class="client-icon">🤖</div>
      <div class="client-text">
        <strong>Claude Code 클라이언트</strong><br/>
        <span style="font-size: 0.875rem; opacity: 0.9;">POST /v1/messages</span>
      </div>
    </div>
  </div>

  <div class="arch-connector">↓</div>

  <div class="arch-layer">
    <div class="arch-layer-title">프록시 엔진</div>
    <div class="arch-proxy">
      <div class="arch-proxy-component">🔐 인증</div>
      <div class="arch-proxy-component">📝 요청 로깅</div>
      <div class="arch-proxy-component">📡 SSE 스트리밍</div>
      <div class="arch-proxy-component">🔑 API 키 관리</div>
    </div>
  </div>

  <div class="arch-connector">↓</div>

  <div class="arch-layer">
    <div class="arch-layer-title">프로바이더 레이어 (구현됨)</div>
    <div class="arch-providers">
      <div class="arch-provider anthropic">
        <img src="/logos/anthropic.svg" alt="Anthropic" class="arch-provider-logo" />
        <div class="arch-provider-name">Anthropic</div>
        <div class="arch-provider-desc">Claude 모델</div>
      </div>
      <div class="arch-provider zai">
        <img src="/logos/zai.svg" alt="Z.AI" class="arch-provider-logo" />
        <div class="arch-provider-name">Z.AI</div>
        <div class="arch-provider-desc">GLM 모델</div>
      </div>
    </div>
  </div>

  <div class="arch-connector" style="margin-top: 1rem;">↓</div>

  <div class="arch-layer">
    <div class="arch-layer-title" style="opacity: 0.7;">출시 예정</div>
    <div class="arch-providers" style="opacity: 0.6;">
      <div class="arch-provider ollama">
        <img src="/logos/ollama.svg" alt="Ollama" class="arch-provider-logo" />
        <div class="arch-provider-name">Ollama</div>
        <div class="arch-provider-desc">로컬 모델</div>
      </div>
      <div class="arch-provider bedrock">
        <img src="/logos/aws.svg" alt="AWS Bedrock" class="arch-provider-logo" />
        <div class="arch-provider-name">AWS Bedrock</div>
        <div class="arch-provider-desc">SigV4 인증</div>
      </div>
      <div class="arch-provider azure">
        <img src="/logos/azure.svg" alt="Azure" class="arch-provider-logo" />
        <div class="arch-provider-name">Azure Foundry</div>
        <div class="arch-provider-desc">배포</div>
      </div>
      <div class="arch-provider vertex">
        <img src="/logos/gcp.svg" alt="Vertex AI" class="arch-provider-logo" />
        <div class="arch-provider-name">Vertex AI</div>
        <div class="arch-provider-desc">OAuth</div>
      </div>
    </div>
  </div>
</div>
</div>

<div class="section-box">
  <h2 class="section-title">사용 사례</h2>
  <p class="section-description">CC-Relay로 개발 워크플로우를 강화하세요</p>

  <div class="use-cases-grid">
    <div class="use-case-card">
      <div class="use-case-icon">🔄</div>
      <h3>프로바이더 유연성</h3>
      <p>클라이언트 코드 변경 없이 Anthropic과 Z.AI 간 전환</p>
    </div>
    <div class="use-case-card">
      <div class="use-case-icon">👥</div>
      <h3>개발 팀</h3>
      <p>풀링된 키로 여러 개발자 간 API 할당량 공유</p>
    </div>
    <div class="use-case-card">
      <div class="use-case-icon">🔑</div>
      <h3>API 키 관리</h3>
      <p>클라이언트 업데이트 없이 API 키 중앙 관리 및 로테이션</p>
    </div>
    <div class="use-case-card">
      <div class="use-case-icon">💰</div>
      <h3>비용 비교</h3>
      <p>저렴한 대안으로 Z.AI의 GLM 모델 테스트</p>
    </div>
    <div class="use-case-card">
      <div class="use-case-icon">🔐</div>
      <h3>구독 패스스루</h3>
      <p>Claude Code 구독 사용자를 프록시를 통해 라우팅</p>
    </div>
    <div class="use-case-card">
      <div class="use-case-icon">🐛</div>
      <h3>요청 디버깅</h3>
      <p>문제 해결을 위한 API 요청 로깅 및 검사</p>
    </div>
  </div>
</div>

<div class="section-box">
  <h2 class="section-title">문서</h2>
  <p class="section-description">CC-Relay 시작 및 마스터에 필요한 모든 것</p>

  <div class="docs-grid">
    {{< hextra/feature-card
      title="시작하기"
      subtitle="설치, 설정 및 첫 실행"
      link="/ko/docs/getting-started/"
    >}}
    {{< hextra/feature-card
      title="설정"
      subtitle="프로바이더 설정, 라우팅 전략 및 고급 옵션"
      link="/ko/docs/configuration/"
    >}}
    {{< hextra/feature-card
      title="아키텍처"
      subtitle="시스템 설계, 컴포넌트 및 API 호환성"
      link="/ko/docs/architecture/"
    >}}
    {{< hextra/feature-card
      title="API 레퍼런스"
      subtitle="HTTP 엔드포인트, 스트리밍 및 클라이언트 예제"
      link="/ko/docs/api/"
    >}}
  </div>
</div>

<div class="section-box">
  <h2 class="section-title">기여하기</h2>
  <p class="section-description">CC-Relay는 오픈 소스입니다! 기여를 환영합니다</p>

  <div class="contributing-links">
    <a href="https://github.com/omarluq/cc-relay/issues" class="contrib-link">
      <span class="contrib-icon">🐛</span>
      <span>버그 신고</span>
    </a>
    <a href="https://github.com/omarluq/cc-relay/issues" class="contrib-link">
      <span class="contrib-icon">💡</span>
      <span>기능 요청</span>
    </a>
    <a href="https://github.com/omarluq/cc-relay/pulls" class="contrib-link">
      <span class="contrib-icon">🚀</span>
      <span>PR 제출</span>
    </a>
  </div>

  <div class="license-box">
    <p>AGPL 3 라이선스 - 자세한 내용은 <a href="https://github.com/omarluq/cc-relay/blob/main/LICENSE">LICENSE</a>를 참조하세요</p>
  </div>
</div>

<div class="custom-footer">
  <div class="footer-social">
    <a href="https://github.com/omarluq" target="_blank" rel="noopener" aria-label="GitHub">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>
    </a>
    <a href="https://linkedin.com/in/omarluq" target="_blank" rel="noopener" aria-label="LinkedIn">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
      </svg>
    </a>
    <a href="https://bsky.app/profile/omarluq.bsky.social" target="_blank" rel="noopener" aria-label="Bluesky">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 10.8c-1.087-2.114-4.046-6.053-6.798-7.995C2.566.944 1.561 1.266.902 1.565.139 1.908 0 3.08 0 3.768c0 .69.378 5.65.624 6.479.815 2.736 3.713 3.66 6.383 3.364.136-.02.275-.038.415-.056-.138.022-.276.04-.415.056-3.912.58-7.387 2.005-2.83 7.078 5.013 5.19 6.87-1.113 7.823-4.308.953 3.195 2.05 9.271 7.733 4.308 4.267-4.308 1.172-6.498-2.74-7.078a8.741 8.741 0 0 1-.415-.056c.14.018.279.036.415.056 2.67.297 5.568-.628 6.383-3.364.246-.828.624-5.79.624-6.478 0-.69-.139-1.861-.902-2.206-.659-.298-1.664-.62-4.3 1.24C16.046 4.748 13.087 8.687 12 10.8z"/>
      </svg>
    </a>
  </div>
  <p class="footer-powered"><a href="https://gohugo.io" target="_blank" rel="noopener">Hugo</a>로 제작</p>
  <p class="footer-copyright">© 2026 Omar Alani. All rights reserved.</p>
</div>

</div><!-- End .landing-page -->
