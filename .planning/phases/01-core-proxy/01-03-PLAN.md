...
- Handle errors with Anthropic-format error responses
...
```
  ...
  <done>Handler compiles, uses Rewrite function, FlushInterval: -1, forwards headers correctly</done>
</task>
...
```
  ...
  <done>All handler and SSE tests pass, tool_use_id preservation verified</done>
</task>
...
- [ ] Handler returns Anthropic-format errors on failure
