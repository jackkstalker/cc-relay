...
- `ServerConfig` struct: Listen (string), TimeoutMS (int)
...
```
  ...
  <done>Config structs compile, Load function parses example.yaml server/providers/logging sections</done>
</task>
...
```
  ...
  <done>All unit tests pass, coverage for config loading and provider authentication</done>
</task>
...
# Config loads example.yaml without error (manual verification)
# Provider interface is implemented by AnthropicProvider (compile-time verification)
```
</verification>
